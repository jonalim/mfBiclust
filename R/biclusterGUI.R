    ...
    ui = fluidPage(
      ...
      server = function(input, output, session) {
        ...
        }, 
        {
          ...
          values$bce <- if(input$sampleCols) { 
            BiclusterExperiment(as.matrix(reactiveDF()))
          } else { BiclusterExperiment(t(as.matrix(reactiveDF())))
          }
        })
        ...
        #   m <- as.matrix(values$bce)
        #   return(numericInput("k", "Number biclusters", value = 3, min = 1,
        #                max = min(nrow(m), ncol(m)), step = 1))
        # })
        observeEvent(input$bicluster, {
          ...
          if(length(matchStrats) == 0) {
            # Create a new BiclusterStrategy
